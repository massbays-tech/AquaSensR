...
The metadata file includes various information for the quality control checks applied to each parameter. Use `readASRmeta()` to import the QC threshold metadata. The function reads the workbook, runs checks via `checkASRmeta()`, and returns a formatted data frame.
