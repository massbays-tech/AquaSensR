...
3. **External overlay data**: a second file in the same format as the continuous monitoring data, imported the same way (via `readASRcont()`), used only for reference overlay plotting in `editASRflag()` (see the [quality control vignette](qcoverview.html)).

The DQO file is an Excel workbook (`.xlsx`). The continuous monitoring data file (and any external overlay file) can each be an Excel workbook (`.xlsx`), a CSV file (`.csv`), or a comma-delimited text file (`.txt`). This vignette describes how to import and check each required input dataset. It is critical that the input datasets follow the exact specified format. Example files with the correct format are included with the package and are used throughout.
...
If you also have a second file for optional overlay use in `editASRflag()` (see [Overlay data for editASRflag()]), specify its path the same way:
...
As for the other files, the examples below use a file included with the package:
...
`editASRflag()` (see the [quality control vignette](qcoverview.html)) accepts an optional `ext` argument as a second file used only for reference overlay plotting in the app. Because it is plotted alongside `contdat` rather than checked or flagged itself, it is imported exactly like continuous monitoring data, with `readASRcont()`. Any `paramsASR` parameter is allowed, and the file may have one or more parameter columns.
...
Passing `extdat` to `editASRflag(contdat, dqodat, ext = extdat)` adds one entry per column in `extdat` to the app's **Overlay** drop-down, labeled with an "[External File]" suffix. See the [quality control vignette](qcoverview.html) for details on how the overlay is used.

The remaining functions in AquaSensR can now be used after the continuous data and data quality objectives files are successfully imported. The external overlay data, if used, is imported the same way but is not required for any other function.