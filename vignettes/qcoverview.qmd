...
An optional `flow` argument accepts a flow or stage height data frame from `readASRflow()` (see the [inputs vignette](inputs.html)), which adds it as an entry in the **Overlay** control described below.
...
If a `flow` data frame was supplied to `editASRflag()`, its column appears as an additional entry in the **Overlay** drop-down (labeled with a "[Flow File]" suffix), alongside the `contdat` parameters.  Its `DateTime` values are aligned to `contdat`'s time zone and clipped to `contdat`'s date range before display. The entry is omitted if the two do not overlap at all.  Unlike the USGS Overlay, no network request is made since the data was already loaded via `readASRflow()`.
