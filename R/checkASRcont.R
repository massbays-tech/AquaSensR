...
checkASRcont <- function(contdat, warn = TRUE){
  ...
  chk <- parms %in% nms
  if(!any(chk)){
    stop(msg, '\n\tNo parameter columns found. Please include at least one of the following columns: ', paste(parms, collapse = ', '), call. = FALSE)
  }
  message(paste(msg, 'OK'))
  ...
  if(any(is.na(chk))){
    ...
    stop(msg, '\n\tThe following rows have dates that are not in a recognizable format: ', paste(tochk, collapse = ', '), call. = FALSE)
  }
  message(paste(msg, 'OK'))
  ...
  if(any(is.na(chk))){
    tochk <- which(is.na(chk))
    stop(msg, '\n\tThe following rows have times that are not in a recognizable format: ', paste(tochk, collapse = ', '), call. = FALSE)
  }
  message(paste(msg, 'OK'))
  ...
  chk <- sapply(nms, function(x) suppressWarnings(as.numeric(contdat[[x]])) |> is.na() |> any())
  if(any(chk)){
    # get rows with non-numeric values
    tochk <- lapply(nms[chk], function(x){
      which(suppressWarnings(as.numeric(contdat[[x]])) |> is.na())
    })
    stop(msg, '\n\tThe following parameter columns have non-numeric values in the following rows: ', paste(sapply(names(tochk), function(x) paste0(x, ' (', paste(tochk[[x]], collapse = ', '), ')')), collapse = '; '), call. = FALSE)
  }
  message(paste(msg, 'OK'))
