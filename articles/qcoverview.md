...
- [`utilASRflag()`](https://massbays-tech.github.io/AquaSensR/reference/utilASRflag.md)
  — applies four independent QC checks to a selected parameter and
  returns a data frame of flag results.
...
- Values below `GrMinFail` or above `GrMaxFail` → `"fail"`
...
- Run length ≥ `FlatSuspectN` (using `FlatSuspectDelta` tolerance) →
  `"suspect"`
- Run length ≥ `FlatFailN` (using `FlatFailDelta` tolerance) → `"fail"`
...
[`anlzASRflag()`](https://massbays-tech.github.io/AquaSensR/reference/anlzASRflag.md)
function. The produces an interactive time-series plot:

``` r
anlzASRflag(flagdat)
```

The plot shows all observations as a continuous grey line. Non-passing
observations are overlaid as coloured markers, with colour encoding the
check that and shape encoding the severity:
